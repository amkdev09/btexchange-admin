import api from '../utils/axios2';

const BASE_URL = '/admin';

const networkService = {
  signin: async (credentials) => {
    const response = await api.post(`${BASE_URL}/signin`, credentials);
    return response.data;
  },

  // Dashboard
  getDashboard: async () => {
    const response = await api.get(`${BASE_URL}/dashboard`);
    return response.data;
  },

  // User Management
  getUsers: async (params = {}) => {
    const response = await api.get(`${BASE_URL}/users`, { params });
    return response.data;
  },

  getUserDetails: async (userId) => {
    const response = await api.get(`${BASE_URL}/user/${userId}`);
    return response.data;
  },

  blockUnblockUser: async (userId, data) => {
    const response = await api.put(`${BASE_URL}/user/${userId}/block-unblock`, data);
    return response.data;
  },

  // Financial Management
  getIncomeHistory: async (params = {}) => {
    const response = await api.get(`${BASE_URL}/income-history`, { params });
    return response.data;
  },

  getDepositHistory: async (params = {}) => {
    const response = await api.get(`${BASE_URL}/deposit-history`, { params });
    return response.data;
  },

  getWithdrawalHistory: async (params = {}) => {
    const response = await api.get(`${BASE_URL}/withdrawal-history`, { params });
    return response.data;
  },

  // Fund Management
  getAllFunds: async (params = {}) => {
    const response = await api.get(`${BASE_URL}/funds/all`, { params });
    return response.data;
  },

  sweepAddress: async (data) => {
    const response = await api.post(`${BASE_URL}/funds/sweep-address`, data);
    return response.data;
  },

  sweepAll: async (data) => {
    const response = await api.post(`${BASE_URL}/funds/sweep-all`, data);
    return response.data;
  },
};

export default networkService;
